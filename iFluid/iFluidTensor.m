...
methods (Access = public)
    ...
    function obj = iFluidTensor(varargin)
        ...
        if length(varargin) == 1 % input is matrix
            obj.matrix = varargin{1};
            assert( ndims(obj.matrix) <= 5, 'iFluidTensor can have max 5 dimensions!' )
        else
            if ischar(varargin{end})
                assert( length(varargin) <= 6, 'iFluidTensor can have max 5 dimensions!' )
                if strcmp(varargin{end}, 'zeros')
                    obj.matrix = zeros(varargin{1:end-1});
                elseif strcmp(varargin{end}, 'ones')
                    obj.matrix = ones(varargin{1:end-1});
                elseif strcmp(varargin{end}, 'eye')
                    assert( length(varargin) == 6, 'Must be 5-dimensional!')
                    
                    I1 = eye(varargin{1}, varargin{4});
                    I2 = eye(varargin{3}, varargin{5});
        
                    I1 = permute( I1, [1 4 3 2] );
                    I2 = permute( I2, [3 5 1 4 2] );
                    I  = I1.*I2;
         
                    obj.matrix = repmat(I, 1, varargin{2});
                end
            else
                
                assert( length(varargin) <= 5, 'iFluidTensor can have max 5 dimensions!' )
