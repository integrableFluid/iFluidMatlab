function [phi, drho, boost] = sample_thermal_flucts_TLL(N_samples, ...      % number of samples
                                                        ...
                                                        coherent_amp, ...   % amplitude of coherent fluctuations
                                                        coherent_angle)     % angle of coherent fluctuations (0 and pi is density, pi/2 and 3pi/2 is phase)
    %
...
%         n_j         = T/eps_k; % Reighley-Jeans
        ...
        X           = randn(N_samples, 2);        
        b_j         = coherent_amp(j)*exp(1i*coherent_angle(j)) + sqrt(n_j/2)*(X(:,1)+1i*X(:,2));
