classdef CollisionSolverFILLING < iFluidSolver
...
methods (Access = public)
    ...
    function obj = CollisionSolverFILLING(coreObj, lperp, Options)        
        ...
        obj.P1 = 2*Pm.*2.*k.*heaviside( 2*k*lperp-2*sqrt(2) );
        obj.P2 = 2*Pp.*2.*k;
    ...
    function [I, J] = calcCollisionIntegral(obj, t, theta, nu)
        ...
        Iint    = obj.P1.*rhoS.t().*(-theta.*theta.t().*(1-theta_m).*(1-theta_m.t()) + ...
                                    (1-theta).*(1-theta.t()).*theta_m.*theta_m.t().*nu)+ ...
                  obj.P2.*rhoS.t().*(-theta.*theta.t().*(1-theta_p).*(1-theta_p.t()).*nu + ...
                                    (1-theta).*(1-theta.t()).*theta_p.*theta_p.t() );
        ...
        Jint    = (obj.P2/(2*Nat)).*rhoS.*rhoS.t().*((1-theta).*(1-theta.t()).*theta_p.*theta_p.t() - ...
                                                     theta.*theta.t().*(1-theta_p).*(1-theta_p.t()).*nu );
